...
class IntegerFieldTest extends Orchestra\Testbench\TestCase
{
	...
	public function testUpdateAttribute()
	{
		...
		$params = [
			'admin_label' => 'integer_attribute_changed'
		];

		$app = $this->createApplication();
		$bus = $app->make('Illuminate\Contracts\Bus\Dispatcher');
		
		$result = $bus->dispatch( new Congraph\Eav\Commands\Attributes\AttributeUpdateCommand($params, 10) );
		
		// $this->d->dump($result->toArray());

		$this->assertTrue($result instanceof Congraph\Core\Repositories\Model);
		$this->assertTrue(is_int($result->id));
		$this->assertEquals($result->admin_label, 'integer_attribute_changed');
	...
	public function testFilterEntity()
	{
		...
		$result = $bus->dispatch( new Congraph\Eav\Commands\Entities\EntityGetCommand(['filter' => ['fields.test_integer_attribute' => '11']]));
		// $this->d->dump($result->toArray());
		...
		$result = $bus->dispatch( new Congraph\Eav\Commands\Entities\EntityGetCommand(['filter' => ['fields.test_integer_attribute' => ['in' => '11']]]));
		// $this->d->dump($result->toArray());
		$this->assertTrue($result instanceof Congraph\Core\Repositories\Collection);
		$this->assertEquals(1, count($result));
		$this->assertEquals(11, $result[0]->fields->test_integer_attribute);

		$result = $bus->dispatch( new Congraph\Eav\Commands\Entities\EntityGetCommand(['filter' => ['fields.test_integer_attribute' => ['nin' => '11']]]));
		// $this->d->dump($result->toArray());
		$this->assertTrue($result instanceof Congraph\Core\Repositories\Collection);
		$this->assertEquals(0, count($result));

		$result = $bus->dispatch( new Congraph\Eav\Commands\Entities\EntityGetCommand(['filter' => ['fields.test_integer_attribute' => ['lt' => '11']]]));
		// $this->d->dump($result->toArray());
		$this->assertTrue($result instanceof Congraph\Core\Repositories\Collection);
		$this->assertEquals(0, count($result));

		$result = $bus->dispatch( new Congraph\Eav\Commands\Entities\EntityGetCommand(['filter' => ['fields.test_integer_attribute' => ['lte' => '11']]]));
		// $this->d->dump($result->toArray());
		$this->assertTrue($result instanceof Congraph\Core\Repositories\Collection);
		$this->assertEquals(1, count($result));
		$this->assertEquals(11, $result[0]->fields->test_integer_attribute);
		...
		$result = $bus->dispatch( new Congraph\Eav\Commands\Entities\EntityGetCommand(['filter' => ['fields.test_integer_attribute' => ['gt' => '11']]]));
		// $this->d->dump($result->toArray());
		$this->assertTrue($result instanceof Congraph\Core\Repositories\Collection);
		$this->assertEquals(0, count($result));

		$result = $bus->dispatch( new Congraph\Eav\Commands\Entities\EntityGetCommand(['filter' => ['fields.test_integer_attribute' => ['gte' => '11']]]));
		// $this->d->dump($result->toArray());
		$this->assertTrue($result instanceof Congraph\Core\Repositories\Collection);
		$this->assertEquals(1, count($result));
		$this->assertEquals(11, $result[0]->fields->test_integer_attribute);
